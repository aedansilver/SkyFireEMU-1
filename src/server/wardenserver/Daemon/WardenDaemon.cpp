...
void PacketHandler(BYTE *pPacket, DWORD dwSize, DWORD *dwBuffer, Instances *instance)
{
    ...
    sLog->outStaticDebug("Warden::PacketHandler handled %u bytes of the packet", *dwBuffer);
}
...
void WardenServer::Initialize()
{
    sLog->outString("Warden anti-cheat system initialized." );
}
...
bool WardenServer::LoadModuleAndExecute(uint32 accountId, uint32 modLen, uint8 *module, uint8 *sessionKey, uint8 *packet, ByteBuffer *returnPacket)
{
    sLog->outStaticDebug("WardenServer::LoadModuleAndExecute()");

    uint32 m_signature = *(uint32*)(module + modLen - 4); // - sizeof(uint32)
    if (m_signature != 0x5349474E) // NGIS->SIGN string
    {
        sLog->outError("Warden module has been corrupted, system cannot find signature data.");
        return false;
    }
...
uint8 *WardenServer::GenerateNewKeys(Instances *instance, uint8 *sessionKey/*[40]*/, uint8 *packet/*[17]*/)
{
    sLog->outStaticDebug("WardenServer::GenerateNewKeys()");
