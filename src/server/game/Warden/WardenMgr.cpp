...
WardenMgr::WardenMgr() : _Disconnected(false), _Banning(false), _HalfCall(false)
{
...
void WardenMgr::Initialize(const char *addr, u_short port, bool IsBanning)
{
    ...
    if (!LoadFromDB())
    {
        sLog->outError("Warden was disabled because there is no usable module or 1 table has no record");
        _Enabled = false;
        return;
    }

    if (_Disconnected = !InitializeCommunication())
    {
        sLog->outError("The warden server was not reachable, trying to reconnect in the background");
        _PingOut = true;
    }
...
bool WardenMgr::InitializeCommunication()
{
    ...
    ACE_INET_Addr remoteAddr(_WardenPort, _WardenAddress.c_str());
    if (_Connector.connect(handler, remoteAddr) == -1)
    {
        return false;
    }

    _WardenProcessStream = handler->Peer;
    ByteBuffer pkt;
    const char *sign = WARDEN_SIGN;
    pkt << sign;
    _WardenProcessStream->send((char const*)pkt.contents(), pkt.size());
...
void WardenMgr::Update(uint32 diff)
{
    ...
    {
        if (_PingOut && _Disconnected)
        {
            _Disconnected = !InitializeCommunication();
            m_PingTimer.Reset();
        }
        else if (_PingOut && !_Disconnected)
        {
            SetDisconnected();
        }
        else
...
{
    _HalfCall = !_HalfCall;                           // To return half of the time since called 2 times
    if (!_HalfCall)
        return;

    if (session->m_WardenTimer.Passed())                // We don't care the connection to wardenserver state to do cheat-checks or register
    {
    ...
    if (_Disconnected)
    {
        ...
        if (session->m_wardenStatus == WARD_STATE_PENDING_WARDEN)
            session->m_wardenStatus = WARD_STATE_NEED_WARDEN;     // We needed data, so have to redo the request
        return;
    }

    // Connected to warden-server, last time was disconnected, then resume and re-ask for module load and key generation
    if (session->m_WardenTimer.Passed() && session->m_wardenStatus == WARD_STATE_NEED_WARDEN)
    {
        LoadModuleAndGetKeys(session);
        session->m_WardenTimer.SetInterval(10 * IN_MILLISECONDS);
        session->m_WardenTimer.Reset();
        session->m_wardenStatus = WARD_STATE_PENDING_WARDEN;
    }
}
...
void WardenMgr::SetDisconnected()
{
    if (!_Disconnected)
    {
        sLog->outError("Connection to Warden server was lost, trying to reconnect in the background");
        _Connector.close();
...
bool WardenMgr::LoadFromDB()
{
    ...
    uint32 count = 0;
    {
        do
        {
            ...
            {
                WardenCheckMap& moduleCheck = _WardenModuleMap[md5];
    ...
    else
    {
        uint32 count = 0;
        {
            _WardenMemoryChecks.resize((int)result->GetRowCount());
            do
            {
                Field *fields = result->Fetch();

                MemoryCheckEntry& current = _WardenMemoryChecks[count];
    ...
    else
    {
        uint32 count = 0;
        {
            _WardenPageChecks.resize((int)result->GetRowCount());
            do
            {
                Field *fields = result->Fetch();

                PageCheckEntry& current = _WardenPageChecks[count];
    ...
    else
    {
        uint32 count = 0;
        {
            _WardenFileChecks.resize((int)result->GetRowCount());
            do
            {
                Field *fields = result->Fetch();

                FileCheckEntry& current = _WardenFileChecks[count];
                current.String  = fields[0].GetString();
                std::string res = fields[1].GetString();
                hexDecodeString(res.c_str(), 40, current.SHA);
    ...
    else
    {
        uint32 count = 0;
        {
            _WardenLuaChecks.resize((int)result->GetRowCount());
            do
            {
                Field *fields = result->Fetch();

                LuaCheckEntry& current = _WardenLuaChecks[count];
                current.String = fields[0].GetString();
    ...
    else
    {
        uint32 count = 0;
        {
            _WardenDriverChecks.resize((int)result->GetRowCount());
            do
            {
                Field *fields = result->Fetch();

                DriverCheckEntry& current = _WardenDriverChecks[count];
                current.Seed    = fields[0].GetUInt32();
                std::string res = fields[1].GetString();
                hexDecodeString(res.c_str(), 40, current.SHA);
...
void WardenMgr::StartForSession(WorldSession* const session)
{
    ...
    QueryResult result = LoginDatabase.PQuery("SELECT module_day, last_module FROM account WHERE id = '%u'", session->GetAccountId());
    if (result)
    {
        Field* fields = result->Fetch();
        uint16 modDay = fields[0].GetUInt16();
        uint32 os = fields[2].GetUInt32();
        /*if (os != 0x0057696E)       // 0x0057696E = \0niW => 'Win' so not windows, not coded yet for mac modules sending
        {
...
void WardenMgr::RandAModuleMd5(std::string *result)
{
    ...
    for (WardenModuleMap::const_iterator itr = _WardenModuleMap.begin(); itr != _WardenModuleMap.end(); ++itr)
    {
...
{
    sLog->outStaticDebug("WardenMgr::SendSeedAndComputeKeys: building wardenserver packet");
    BigNumber s;
    s.SetRand(16 * 8);
    // save this seed for client send later when we have the new keys from warden-server
    memcpy(&session->m_wardenSeed[0], s.AsByteArray(16), 16);
    // build the packet for warden-server only
    LoadModuleAndGetKeys(session);
...
{
    return &_WardenMemoryChecks[urand(0, _WardenMemoryChecks.size()-1)];
}
WardenMgr::PageCheckEntry *WardenMgr::GetRandPageCheck()
{
    return &_WardenPageChecks[urand(0, _WardenPageChecks.size()-1)];
}
WardenMgr::FileCheckEntry *WardenMgr::GetRandFileCheck()
{
    return &_WardenFileChecks[urand(0, _WardenFileChecks.size()-1)];
}
WardenMgr::LuaCheckEntry *WardenMgr::GetRandLuaCheck()
{
    return &_WardenLuaChecks[urand(0, _WardenLuaChecks.size()-1)];
}
WardenMgr::DriverCheckEntry *WardenMgr::GetRandDriverCheck()
{
    return &_WardenDriverChecks[urand(0, _WardenDriverChecks.size()-1)];
}

void WardenMgr::SendCheatCheck(WorldSession* const session)
{
    sLog->outStaticDebug("WardenServer::BuildCheatCheck(%u, *pkt)", session->GetAccountId());
    ...
    sLog->outStaticDebug("Seed byte: 0x%02X, end byte: 0x%02X", m_seed1, _WardenModuleMap[md5][WARD_CHECK_END]);
    ...
    data << uint8(_WardenModuleMap[md5][WARD_CHECK_TIMING] ^ m_seed1);
    // Finally put the other checks
    uint8 m_strIndex = 1;
    sLog->outStaticDebug("Preparing %u checks", nbChecks);
    for (uint8 i=0; i<nbChecks; ++i)
    {
        data << uint8(_WardenModuleMap[md5][(*checkList)[i].check] ^ m_seed1);
    ...
    data << uint8(_WardenModuleMap[md5][WARD_CHECK_END] ^ m_seed1);
...
bool WardenMgr::ValidateCheatCheckResult(WorldSession* const session, WorldPacket& clientPacket)
{
    ...
    sLog->outStaticDebug("WardenServer::ValidateCheatCheckResult(%u, *pkt)", accountId);
    bool valid = true;
    ...
    for (uint8 i=0; i<checkList->size(); ++i)
    {
        ...
        switch ((*checkList)[i].check)
        {
            ...
            case WARD_CHECK_MEMORY:
            {
                ...
                {
                    localCheck = false;
                    sLog->outError("Kicking account %u for failed check, MEM at Offset 0x%04X, length %u could not be read by client", accountId, (*checkList)[i].mem->Offset, (*checkList)[i].mem->Length);
                }
                else
                {
                    uint8 memContent[20];
            ...
            default:
                ...
                // Finish skipping the rest of the packet and return failed checks
                sLog->outBasic("Wrong packet for account %u or problem to parse it, Warden had to clean %u bytes", accountId, clientPacket.size() - clientPacket.rpos());
...
int WardenSvcHandler::handle_input(ACE_HANDLE /*handle*/)
{
    ...
    {
        uint8 _trash[1024];
        sLog->outError("Unexpected packet [%u] from Warden server, trashing it", _cmd);
